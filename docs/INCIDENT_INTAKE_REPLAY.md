...
계속 표시한다. 합성 사고 좌표는 공개 소방서 좌표에서 대한민국 중심 방향 약 1.2km 지점으로
계산하여 모든 소방서에서 출동 기준점과 겹치지 않으면서도 인접 범위를 유지한다. 실제 관할
경계나 도로 주소를 주장하지 않으며 주소 문자열은 `관할 공개 합성 사고지점`으로 고정한다.
