...
grep -q -- '--arg confirmedCount "$confirmation_index"' "${DEPLOY_SCRIPT}"
grep -q '.confirmedCount == ($confirmedCount | tonumber)' "${DEPLOY_SCRIPT}"
grep -q -- '--arg latitude "$pilot_latitude"' "${DEPLOY_SCRIPT}"
grep -q 'latitude: ($latitude | tonumber)' "${DEPLOY_SCRIPT}"
grep -q 'CHEMICHECK119_PHONE_INGRESS_ENABLED=' "${DEPLOY_SCRIPT}"
grep -q 'CHEMICHECK119_PHONE_INGRESS_TOKEN=' "${DEPLOY_SCRIPT}"
grep -q 'GCP_PROMOTE_TRAFFIC' "${DEPLOY_SCRIPT}"
grep -q 'traffic promotion was intentionally skipped' "${DEPLOY_SCRIPT}"
grep -q '&& \[ -n "$session_cookie_file" \]' "${DEPLOY_SCRIPT}"
grep -q 'Phone ingress authentication smoke failed' "${DEPLOY_SCRIPT}"
