...
required_variables=(
  ...
  GCP_SPEECH_API_KEY_SECRET_VERSION
  GCP_SESSION_SECRET
...
[[ "$GCP_SPEECH_API_KEY_SECRET_VERSION" =~ ^[1-9][0-9]*$ ]]
...
[[ "$GCP_SPEECH_API_BASE_URL" =~ ^https://[a-z0-9.-]+\.run\.app/?$ ]]
[[ "$GCP_MODEL_API_KEY_SECRET" =~ ^[a-zA-Z0-9_-]+$ ]]
[[ "$GCP_SPEECH_API_KEY_SECRET" =~ ^[a-zA-Z0-9_-]+$ ]]
...
smoke() {
  ...
  if [ "$GCP_PUBLIC_PILOT_ACCESS_ENABLED" = "true" ]; then
    ...
    local speech_smoke_file
    local speech_request_id="REQ-DEPLOY-SPEECH-${RELEASE_GIT_COMMIT:0:8}"
    speech_smoke_file="$(mktemp)"
    python3 - "$speech_smoke_file" <<'PY'
import sys
import wave
...
PY
    http_code="$(curl --silent --show-error \
      ...
      --header "X-Request-Id: $speech_request_id" \
      --cookie "$session_cookie_file" \
      --data-binary "@$speech_smoke_file" \
      "$base_url/api/c2guard/v1/transcriptions")"
    rm -f "$speech_smoke_file"
    if [ "$http_code" != "200" ] || ! jq --exit-status \
      --arg requestId "$speech_request_id" \
      '.schemaVersion == "chemicheck119-dashboard-bff-v1"
        and .requestId == $requestId
        and .incidentId == null
        and (.status == "TRANSCRIBED" or .status == "ABSTAINED_NO_TRANSCRIPT")
        and .requiresResponderReview == true
        and .input.audioRetained == false
        and .runtime.hotwordsUsed == false
        and .safetyBoundary.uncertaintyPreserved == true
        and .safetyBoundary.qualitySignalsAreCalibratedProbabilities == false
        and .safetyBoundary.chemicalIdentificationPerformed == false
        and .safetyBoundary.casConfirmationPerformed == false
        and .safetyBoundary.riskAssessmentPerformed == false
        and .safetyBoundary.decisionSupportOnly == true' \
      "$health_file" >/dev/null; then
      echo "Authenticated BFF to private Speech API smoke failed: HTTP $http_code"
      rm -f "$session_cookie_file" "$health_file"
      return 1
    fi
